...
from .models import News, last_News_date, Author, Lawyer
...
# from newsprojects.settings import settings.SIMPLE_JWT
...
class LawyerApiCreated(generics.GenericAPIView):
    serializer_class = LawyerPostSerilizer
    permission_classes = [permissions.IsAuthenticated]

    def get(self,request):
        try:
            lawyer = Lawyer.objects.get(user = request.user)
            return Response(LawyerGetSerilizer(lawyer, context={'request': request}).data, status=status.HTTP_200_OK)
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(LawyerPostSerilizer(serializer.data).data, status=status.HTTP_201_CREATED)
